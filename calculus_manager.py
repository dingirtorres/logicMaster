...
class CalculusManager(multiprocessing.Process):
    ...
    @staticmethod
    def _pack_by_base_name(
        indicator_cache: Dict[str, Any],
        key_to_base_map: Dict[str, str] = None,
    ) -> Dict[str, Dict]:
        """
        Agrupa las claves del indicator_cache por base_name.

        Fuente primaria: key_to_base_map (construido desde los atributos
        .base_name y .output_keys de cada indicador en LogicMaster).
        Esto resuelve el 100% de los casos sin ambigüedad.

        Fallback (si key_to_base_map no cubre una clave): heurística
        de segmentos — recorre desde el final, quita segmentos
        puramente alfabéticos (sufijos funcionales) hasta encontrar
        un segmento con dígitos (frontera paramétrica). Si todos son
        alfabéticos, la clave queda atómica.

        Resultado: dict {base_name: {output_key: value, ...}}
        """
        if not indicator_cache:
            return {}

        if key_to_base_map is None:
            key_to_base_map = {}

        keys = list(indicator_cache.keys())

        def _heuristic_base(key: str) -> str:
            """Fallback heurístico por segmentos con/sin dígitos."""
            parts = key.split("_")
            for i in range(len(parts) - 1, -1, -1):
                if any(c.isdigit() or c == '.' for c in parts[i]):
                    return "_".join(parts[: i + 1])
            return key  # Todos alfabéticos → atómica

        # Paso 1: Resolver base_name para cada clave
        base_to_keys: Dict[str, List[str]] = {}
        for key in keys:
            base = key_to_base_map.get(key) or _heuristic_base(key)
            if base not in base_to_keys:
                base_to_keys[base] = []
            base_to_keys[base].append(key)

        # Paso 2: Construir grupos
        groups: Dict[str, Dict[str, Any]] = {}
        for base, members in base_to_keys.items():
            groups[base] = {k: indicator_cache[k] for k in members}
    ...
    def _process_control_command(self, cmd: Dict[str, Any]) -> None:
        """Ejecuta un comando recibido por la Queue de control."""
        cmd_type = cmd.get("type", "")

        if cmd_type == CMD_WARMUP:
            token_id    = cmd.get("token_id")
            candles     = cmd.get("candles", [])
            skip_series = bool(cmd.get("skip_series", False))
            if not token_id:
                self._logger.error("WARMUP sin token_id.")
                return

            if token_id not in self._contexts:
                self._add_context(token_id)

            self._active_token = token_id
            self._warmup(token_id, candles, skip_series=skip_series)

        elif cmd_type == CMD_RESET:
            token_id = cmd.get("token_id")
            if token_id:
                self._reset_context(token_id)
            else:
                # Reset de todos los contextos
                for tid in list(self._contexts.keys()):
                    self._reset_context(tid)
            self._pipe.send({
                "type": MSG_STATUS,
        ...
        elif cmd_type == CMD_REMOVE_TOKEN:
            token_id = cmd.get("token_id")
            if token_id:
                self._remove_context(token_id)
                self._pipe.send({
                    "type": MSG_STATUS,
        ...
        elif cmd_type == CMD_RECONFIG:
            # Actualizar parámetros de configuración en caliente
            new_config = cmd.get("config", {})
            if new_config:
                self._config.update(new_config)
                self.buffer_size_config = self._config.get(
                    "buffer_size_config",
                    self.buffer_size_config,
                )
                self._logger.info("Reconfiguración aplicada: %s", new_config)

        elif cmd_type == CMD_SHUTDOWN:
            self._logger.info("Comando SHUTDOWN recibido.")
            self._stop_event.set()

        else:
            self._logger.warning("Comando desconocido: %s", cmd_type)
    ...
    def run(self) -> None:
        """
        Bucle principal del proceso CalculusManager.

        Ciclo:
          1. Chequear cola de control (no bloqueante).
          2. Leer del Pipe (con timeout corto).
          3. Procesar tick y enviar ACK.
          4. Repetir hasta stop_event.
        """
        # Configurar logging en el proceso hijo
        self._logger = logging.getLogger("CalculusManager")
        self._logger.setLevel(
            getattr(logging, self._config.get("log_level", "INFO").upper())
        )
        ...
        # Instalar handlers de señales OS para cierre limpio.
        # SIGTERM y SIGINT setean _stop_event en vez de lanzar excepciones,
        # lo que garantiza que _clean_shutdown() siempre se ejecuta via finally.
        import signal as _signal

        def _handle_signal(signum, frame):
            self._logger.info(
                "Señal %d recibida — iniciando cierre ordenado.", signum
            )
            self._stop_event.set()
        ...
        self._logger.info(
            "Proceso CalculusManager iniciado. PID=%d", os.getpid()
        )

        # Reportar PID al MainController
        self._pipe.send({
            "type": MSG_PID_REPORT,
            "pid": os.getpid(),
            "child_pids": {},
        ...
        try:
            while not self._stop_event.is_set():
                # --- 1. Cola de control (no bloqueante) ---
                self._drain_control_queue()

                # Verificar stop después de procesar comandos
                if self._stop_event.is_set():
                    break

                # --- 2. Leer del Pipe (con timeout) ---
                try:
                    if self._pipe.poll(timeout=0.05):
                        msg = self._pipe.recv()
                        self._handle_pipe_message(msg)
                except EOFError:
                    self._logger.warning("Pipe cerrado. Finalizando.")
                    break
                except Exception as e:
                    self._logger.error(
                        "Error leyendo del Pipe: %s", e, exc_info=True
                    )

        except KeyboardInterrupt:
            self._logger.info("KeyboardInterrupt recibido.")
        except Exception as e:
            self._logger.critical(
                "Excepción crítica en bucle principal: %s", e, exc_info=True
            )
            try:
                self._pipe.send({
                    "type": MSG_ERROR,
    ...
    def _drain_control_queue(self) -> None:
        """Drena todos los comandos pendientes de la cola de control."""
        import queue
        while True:
            try:
                ...
                self._logger.error(
                    "Error procesando comando de control: %s", e
                )

    def _handle_pipe_message(self, msg: Dict[str, Any]) -> None:
        """Procesa un mensaje recibido por el Pipe de datos."""
        msg_type = msg.get("type", "")

        if msg_type == MSG_TICK:
            token_id = msg.get("token_id", self._active_token)
            candle = msg.get("candle")

            if not token_id or not candle:
                self._logger.warning(
                    "TICK inválido: token_id=%s, candle=%s",
                    token_id, bool(candle),
                )
                return

            # Si el token no tiene contexto, crear uno automáticamente
            if token_id not in self._contexts:
                self._add_context(token_id)
                self._active_token = token_id

            ack = self._process_tick(token_id, candle)

            # Emitir eventos del LM como MSG_EVENT separados — igual
            # que en el warmup. El controller los recibe en el handler
            # de MSG_EVENT donde se acumulan por token y se enrutan.
            for evt in ack.pop("events", []):
                try:
                    self._pipe.send({
                        "type":     MSG_EVENT,
                        "token_id": token_id,
                    ...
                    self._logger.error(
                        "Error enviando MSG_EVENT para %s: %s", token_id, e
                    )

            try:
                self._pipe.send(ack)
            except Exception as e:
                self._logger.error("Error enviando ACK: %s", e)

        elif msg_type == MSG_SYNC:
            # Sincroniza la posición real del exchange con el LogicMaster.
            # Llamado desde _sync_lm_position / _force_sync_lm del controller.
            token_id  = msg.get("token_id", self._active_token)
            sync_data = msg.get("sync_data", {})
            ctx = self._contexts.get(token_id)
            if ctx is None:
                self._logger.warning(
                    "MSG_SYNC para token sin contexto: %s", token_id
                )
            else:
                try:
                    ctx.logic_master.sync_active_position(sync_data)
                    self._logger.info(
                        "MSG_SYNC procesado para %s — precio=%.5f",
                        token_id,
                        float(sync_data.get("precio_entrada", 0) or 0),
                    )
                except AttributeError:
                    # El LM no implementa sync_active_position — ignorar.
                    self._logger.debug(
                        "LogicMaster para %s no tiene sync_active_position.", token_id
                    )
                except Exception as e:
                    self._logger.warning(
                        "Error en MSG_SYNC para %s: %s", token_id, e
                    )

        else:
            self._logger.debug("Mensaje de Pipe no reconocido: %s", msg_type)
    ...
    def _clean_shutdown(self) -> None:
        """Libera todos los recursos antes de finalizar el proceso."""
        self._logger.info("Iniciando cierre limpio de CalculusManager...")

        # Cerrar todos los contextos
        for token_id in list(self._contexts.keys()):
            try:
                self._remove_context(token_id)
            except Exception as e:
                self._logger.error(
                    "Error cerrando contexto %s: %s", token_id, e
                )

        # Liberar la Queue de control.
        # cancel_join_thread() es crítico: evita que el proceso quede
        # bloqueado esperando que el thread interno de la Queue vacíe
        # su buffer, lo que causaba semáforos huérfanos al recibir SIGTERM.
        try:
        ...
        self._logger.info(
            "CalculusManager finalizado limpiamente. PID=%d", os.getpid()
        )


# ---------------------------------------------------------------------------
# Factory function para el MainController
# ---------------------------------------------------------------------------

def create_calculus_engine(
    config: Dict[str, Any] = None,
    logic_module_path: str = None,
    logic_class_name: str = "LogicMaster",
) -> Tuple[
    ...
    """
    Factory que crea el CalculusManager y sus canales de comunicación.

    Uso en MainController:

        engine, pipe_conn, control_queue, stop_event = create_calculus_engine(
            config={...},
            logic_module_path="/path/to/logicMaster_008_8.py",
        ...
        # Tick a tick (en QTimer callback)
        pipe_conn.send({
            "type": "TICK",
            "token_id": "BTCUSDT",
            "candle": vela_dict,
        ...
        # Leer ACK (no bloqueante, en QTimer)
        if pipe_conn.poll():
            ack = pipe_conn.recv()
            # ack["indicators"] → dict agrupado por base_name
    ...
    Retorna:
        (engine, main_pipe_conn, control_queue, stop_event)
    """
    # Crear canales
    main_conn, engine_conn = multiprocessing.Pipe(duplex=True)
    control_queue = multiprocessing.Queue()
    stop_event = multiprocessing.Event()

    # Instanciar el proceso
    engine = CalculusManager(
        pipe_conn=engine_conn,
    ...
    return engine, main_conn, control_queue, stop_event